...
export default function AmllFullPlayer() {
  ...
  // Responsive: compact (phone) viewport detection, same rule as Layout.tsx
  ...
  // ---------- shared render helpers (used by both desktop & mobile layouts) ----------
  const renderSongInfo = () => (
    <div className="flex items-center justify-between">
      <div className="min-w-0 flex-1 text-left">
        <div className={cn(
          'relative mb-1 overflow-hidden font-bold leading-tight text-white',
          isCompact ? 'min-h-[1.3em] text-lg' : 'min-h-[1.35em] text-2xl'
        )}>
        ...
        <p className={cn('truncate text-white/60', isCompact ? 'text-sm' : 'text-base')}>
          {currentSong?.artist || '未知歌手'}
          {currentSong?.album && <span className="text-white/30"> 路 {currentSong.album}</span>}
        </p>
      </div>
  ...
  const renderProgress = () => (
    <div className={cn(isCompact ? '' : 'mt-6', 'w-full', isCompact ? '' : 'max-w-[360px]')}>
      <Slider.Root
        className="group relative flex h-5 w-full touch-none select-none items-center"
        value={[displayTime]}
        max={durationMs || 100}
  ...
  const renderControls = () => (
    <div className={cn(isCompact ? 'flex items-center justify-center gap-10' : 'mt-5 flex items-center gap-12')}>
      <button
      ...
      >
        <Rewind className={cn(isCompact ? 'h-8 w-8' : 'h-9 w-9')} fill="currentColor" />
      </button>
      ...
      >
        {isLoading ? (
          ...
          <Pause className={cn(isCompact ? 'h-9 w-9' : 'h-8 w-8')} fill="currentColor" />
        ) : (
          <Play className={cn('ml-0.5', isCompact ? 'h-9 w-9' : 'h-8 w-8')} fill="currentColor" />
        )}
      </button>
      ...
      <button
        ...
        disabled={!currentSong}
      >
        <FastForward className={cn(isCompact ? 'h-8 w-8' : 'h-9 w-9')} fill="currentColor" />
      </button>
    </div>
  )
  ...
  const renderTabs = () => (
    <div className={cn('flex items-center overflow-hidden rounded-xl bg-white/[0.07] p-1 text-white/60 ring-1 ring-white/10 backdrop-blur-md', isCompact ? 'mt-1 h-10 w-full' : 'mt-3 h-10 w-full max-w-[300px]')}>
      <button
      ...
      {sideView === 'comments' ? (
        <div className="h-full min-h-0 overflow-hidden rounded-2xl bg-black/18 p-4 ring-1 ring-white/10 backdrop-blur-md">
          <CommentSection
            ...
            maxHeight={isCompact ? 'calc(100vh - 330px)' : 'calc(100vh - 210px)'}
          />
        </div>
      ) : sideView === 'queue' ? (
  ...
  return (
    ...
    >
      ...
      <div className={cn(
        ...
        isCompact ? 'px-4 pt-[max(0.75rem,env(safe-area-inset-top))] pb-2' : 'px-6 pt-14 pb-4'
      )}>
        <button
      ...
      {isCompact ? (
        ...
        <div className="relative z-10 flex min-h-0 flex-1 flex-col overflow-hidden px-4 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
          {/* Cover + song info row */}
          <div className="flex flex-shrink-0 items-center gap-3">
            <div className={cn(
              'h-20 w-20 flex-shrink-0 overflow-hidden rounded-xl shadow-xl transition-all duration-500 ease-out',
          ...
          {/* Lyrics / comments / queue */}
          <div className="relative mt-3 min-h-0 flex-1 rounded-2xl">
            {renderSideView()}
          </div>

          {/* Progress */}
          <div className="mt-2 flex-shrink-0">{renderProgress()}</div>

          {/* Controls */}
          <div className="mt-1 flex-shrink-0">{renderControls()}</div>
